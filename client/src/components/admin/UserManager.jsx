...
import { Shield, User, RefreshCw, ChevronDown, ChevronUp } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        ...
        />
        <button onClick={fetchUsers} className="p-2 text-timber/50 hover:text-timber transition-colors" aria-label="Refresh">
          <RefreshCw size={16} />
        </button>
      </div>
      ...
      {loading ? (
        ...
        <p className="font-sans text-timber/40 text-center py-10">No users found.</p>
      ) : (
        <>
                  ...
                  <tr key={u.id} className="hover:bg-stone/50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <p className="font-medium text-timber">{u.firstName} {u.lastName}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-timber/60">{u.email}</td>
                    <td className="px-4 py-3 text-timber/50">{u.phone || '–'}</td>
                    <td className="px-4 py-3">
                      <span className={`badge text-[10px] ${u.role === 'ADMIN' ? 'bg-forest text-stone' : 'bg-stone text-timber/60'}`}>
                        {u.role}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-timber/60">{u._count?.bookings ?? 0}</td>
                    <td className="px-4 py-3 text-timber/50 whitespace-nowrap">{formatDate(u.createdAt)}</td>
                    <td className="px-4 py-3">
                      <button
                        onClick={() => toggleRole(u.id, u.role)}
                        disabled={!!updating}
                        className={[
                          'flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-xs font-sans font-medium border transition-colors disabled:opacity-50',
                          u.role === 'ADMIN'
